...
public class FHTML {
  ...
  //Substituir por StringBuilder
    private String content = "";            //Conteúdo buscado
  ...
  private int Parsing(boolean status, boolean display) {
    if(this.html != null || this.html.length() != 0) {
      ...
      try {
        ...
        if (status) System.out.println("índice: " + index + "\tchar: " + new_sc.charAt(index));
      ...
      */
      ...
      {
        content = sub.substring(MIN+1, MAX);      //Raspagem
      }
